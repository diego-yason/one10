			...
			<section class="bg-gray-50 p-6 rounded-lg shadow-md">
				...
				<div class="space-y-4">
					{#each uploadedImages as img, i (img.id)}
						...
						>
							<!-- Thumbnail -->
							<img src={img.preview} alt={img.name} class="w-16 h-16 object-cover rounded" />
							...
							<div class="flex items-center gap-2">
								<button
									...
									onclick={() => decreaseCopies(i)}>-</button
								>
								...
								<button
									...
									class="bg-gray-200 px-2 rounded font-bold"
									onclick={() => increaseCopies(i)}>+</button
								>
							</div>

							<!-- TEST-ONLY: Quantity input for automation -->
							<input
								data-testid={'qty-input-' + img.id}
								type="number"
								min="1"
								class="hidden"
