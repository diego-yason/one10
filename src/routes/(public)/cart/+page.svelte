<script>
	...
	import { CartService } from '$lib/firebase';
...
<div class="flex flex-col px-32 py-24 space-y-6">
	...
	{#if cartItems.length === 0}
		<div class="flex flex-col items-center justify-center py-20">
			...
			<a href="/services" class="bg-amber-300 rounded-4xl px-8 py-2 font-bold text-black hover:bg-amber-400 transition">Shop Now</a>
		</div>
	{:else}
			...
			<div class="flex bg-[#D9D9D9] p-4 rounded-lg mt-4 items-center">
				...
				<div class="flex flex-1">
					<div class="flex-col text-left space-y-35">
						<div>
							<h6 class="m-5 font-spaceGrotesk font-bold">{item.type === 'product' ? 'Product' : 'Service'}</h6>
								...
								{#if item.type === 'service'}
									<div class="m-5 text-xs bg-gray-100 p-2 rounded service-details">
										...
										{:else if item.name === 'Printing'}
											...
											{/if}
											<div><b>Drop-off:</b> {item.details.dropoffMode === 'other' ? item.details.dropoffOther : item.details.dropoffMode}</div>
											<div><b>Pick-up:</b> {item.details.pickupMode === 'other' ? item.details.pickupOther : item.details.pickupMode}</div>
										{:else}
											<div>{JSON.stringify(item.details)}</div>
										{/if}
									</div>
								{:else}
									<pre class="m-5 text-xs bg-gray-100 p-2 rounded">{JSON.stringify(item.details, null, 2)}</pre>
								{/if}
							{/if}
						</div>
						<div>
							{#if item.type === 'product'}
								<div class="flex border-1 w-32 m-5 rounded-lg justify-around">
									<button class="px-1.5 basis-1/4 py-3 border-r-[1px] border-black" on:click={() => updateQuantity(i, item.quantity - 1)} disabled={item.quantity <= 1}>-</button>
									<input type="number" min="1" class="w-12 text-center" value={item.quantity} on:input={(e) => updateQuantity(i, +e.target.value)} />
									<button class="px-1.5 basis-1/4 py-3 border-l-[1px] border-black" on:click={() => updateQuantity(i, item.quantity + 1)}>+</button>
								</div>
							{:else}
								<span style="display:block; margin-top:0.5rem;">Qty: {item.quantity}</span>
							{/if}
							<h6 class="text-left font-bold" style="margin-top:0.25rem;">₱{item.price * item.quantity}</h6>
						</div>
					</div>
				</div>
		...
		<div class="flex border-1 rounded-lg my-auto y-100 p-5 text-left font-bold py-15">
			...
			<div class="ml-auto flex gap-4">
				<button class="bg-yellow-400 text-white py-2 px-4 rounded-lg hover:bg-amber-700 transition duration-300" on:click={clearCart}>Clear Cart</button>
				<button class="bg-yellow-400 text-white py-2 px-4 rounded-lg hover:bg-amber-700 transition duration-300">Checkout</button>
			</div>
		</div>
	{/if}
</div>
